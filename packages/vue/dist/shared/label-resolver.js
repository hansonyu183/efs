...
function resolveBuiltinLabel(key, namespaces, locale) {
...
function getI18nContext(instance) {
    ...
    if (efsI18n?.translate) {
        return {
            translator: (key) => efsI18n.translate(key),
            locale,
            config,
        };
    }
    ...
    if (config) {
        return {
            translator: (key) => resolveEfsI18nLabel({ key, config: config }),
            locale,
            config,
        };
    }
