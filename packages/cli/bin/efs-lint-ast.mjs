...
function extractPageType(scriptContent) {
 ...
 let pageType = null
 visitJs(ast, (node) => {
  if (node.type === 'VariableDeclarator' && node.id?.type === 'Identifier' && node.id.name === 'pageType' && node.init?.type === 'StringLiteral') pageType = node.init.value
 })
 return pageType
}
...
${descriptor.scriptSetup?.content || ''}`
 ...
 if (!knownPageTypes.has(pageType)) {
