...
import type { PlatformApp } from '../../model/types/app'
import type { AuthOption, ResModel } from '../../model/types/resource-model'
import { flattenAppMenuNodes, splitResPath } from '../../model/app/navigation-paths'
import { resolveResModel } from '../../model/resource/resolve'
import type { EfsI18nConfig } from '../../model/app/i18n'
import { EFS_I18N_CONTEXT, mergeEfsI18nConfigs, resolveEfsI18nLabel } from '../../model/app/i18n'
import { normalizeEfsPath, useEfsNavigation } from '../../model/app/navigation'
import type { FlatMenuNode } from '../../model/app/navigation-menu'
import {
...
const firstResourcePath = computed(() => sidebarMenus.value.find((item) => item.type === 'item')?.path ?? '')
const nav = useEfsNavigation({
 initialPath: authRuntime.status.value === 'authenticated'
  ? (props.app.main.defaultPath ? `/${props.app.main.defaultPath}` : firstResourcePath.value)
  : '/login',
 loginPath: '/login',
 firstResourcePath: firstResourcePath.value,
})
const currentPath = computed(() => nav.currentPath.value)
// resource model originates from resolveResModel(props.app, currentPath.value, {}) and is memoized per path.
...
// resource model reference: const resolvedMainTitle = computed(() => resourceModel.value?.title || resolvedEmptyTitle.value)
...
watch([isAuthenticated, isLoginRoute, firstResourcePath, currentPath], ([authenticated, loginRoute, firstPath, path]) => {
...
function resolveResourceModel(path: string) {
 ...
 if (!resourceModelCache.has(normalized)) {
  resourceModelCache.set(normalized, resolveResModel(props.app, normalized, {}))
 }
