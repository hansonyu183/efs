...
|---|---|---|---|---|
| `apps/<app-name>/schemas/app.schema.ts` + `composeAppSchema(...)` | P0 | 已成为正式 authoring 主入口 | 是 | 继续补更多标准示例与校验 |
| `baseline + patch` | P0 | 已成为 app schema 的标准建模方式 | 是 | 继续补更多通用 baseline 与 app patch 示例 |
| model runtime | P0 | 已能把最终 schema 接到当前 Vue runtime | 是 | 继续缩小 runtime bridge 面积 |
