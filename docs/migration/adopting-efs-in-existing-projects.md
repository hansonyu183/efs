...
本文用于指导已有企业项目从“本地自定义布局 / 本地自定义 Page”迁移到 EFS。当前迁移主线已经切到 **schema-first**：业务仓库优先维护 `schemas/<app-name>/app.schema.ts`，再由平台固定入口直接加载并接到运行时。
...
- 项目先跑通 `schemas/<app-name>/app.schema.ts -> platform entry -> EfsApp`
...
### 应保留
- `schemas/<app-name>/app.schema.ts` 及相关 schema authoring 文件
...
1. 入口已经切到 `schemas/<app-name>/app.schema.ts`
...
### 新口径
- 业务优先维护 `schemas/<app-name>/app.schema.ts`
