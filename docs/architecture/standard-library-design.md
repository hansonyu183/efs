...
- `apps/tenant-console-web/src/components/查询区.vue`
...
- EntityListTable / SimpleTablePanel / CrudDialog
- - useListPagination
...
- `frontend/web-app/src/components/panels/查询区.vue`
...
2. **共享结构组件更贴近业务实际**
 - 尤其是 `EntityListTable`、`SimpleTablePanel`、`CrudDialog`。
3. **交互细节更完整**
...
### 5.3 查询列表页
- 结构：`PagePanel + 查询区 + EntityListTable`
- 必须组件：EntityListTable、ActionBar、Pagination
- 场景：通用资源查询
...
- 必须组件：ReportPanel、SimpleTablePanel / 
- 场景：统计报表、台账、对账、导出页
...
### 5.11 审批任务页
- 结构：`PagePanel + 查询区 + EntityListTable + DetailPanel/TaskPanel`
- 必须组件：EntityListTable、StatusChip
- 场景：待办、已办、审批处理
...
### 5.12 基础资料页
- 结构：`BaseDataPageShell = 查询区 + EntityListTable + CrudDialog`
- 必须组件：EntityListTable、CrudDialog
- 场景：客户、供应商、员工、物料等
