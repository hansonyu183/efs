...
5. 平台内部持续吸收成熟模式，沉淀为可复用资产，并通过 lint / contract / CI 防止漂移。
...
CompanyOS 已经形成一批可复用度较高的页面壳层组件，主要集中在 `apps/tenant-console-web/src/components/`：
...
- `apps/tenant-console-web/src/components/SimpleTableShell.vue`
- `apps/tenant-console-web/src/components/QueryToolbarShell.vue`
- `apps/tenant-console-web/src/components/CrudDialogShell.vue`
- `apps/tenant-console-web/src/components/AppAlerts.vue`
...
4. **报表/对账/台账页**
   - 以 `SimpleTableShell`、查询区、报表结果区为基础。
5. **工作台页**
   - 权限感知的快捷入口 + 指标概览。
...
- 默认落地页：`/workbench`
- 响应式：移动端卡片、桌面端表格，要求由共享壳层承担
- 组件优先级明确：
  - 列表优先 `EntityListTable`
  - 非分页表优先 `SimpleTableShell`
  - 查询区优先 `QueryToolbarShell`
  - 新增编辑优先 `CrudDialogShell`
- 交互原则：
...
- `apps/tenant-console-web/src/layouts/AuthLayout.vue`
- `apps/tenant-console-web/src/layouts/MainLayout.vue`

#### 2.1.4 平台壳层能力

CompanyOS 的壳层能力较成熟，已不仅是 UI 组件，而是完整前端平台能力：
...
- MainLayout / AuthLayout 壳层模式
- QueryToolbarShell / EntityListTable / SimpleTableShell / CrudDialogShell
- AppAlerts
- useListPagination
...
- 列设置逻辑：目前分散在表格壳层内部，建议上收为统一 `ColumnSettings` 能力
...
对应证据：
- `frontend/web-app/src/components/app/AppAlerts.vue`
- `frontend/web-app/src/components/shell/PageSection.vue`
- `frontend/web-app/src/components/shell/QueryToolbarShell.vue`
- `frontend/web-app/src/components/shell/EntityListTable.vue`
- `frontend/web-app/src/components/shell/SimpleTableShell.vue`
- `frontend/web-app/src/components/shell/CrudDialogShell.vue`
- `frontend/web-app/src/components/shell/DashboardCardShell.vue`
...
对应证据：
- `frontend/web-app/src/app/layouts/AuthLayout.vue`
- `frontend/web-app/src/app/layouts/MainLayout.vue`
...
- 共享 shell 是强制 first-choice primitive
- 运行时元数据页也必须复用同一套壳层
- route shape：
  - `/login`
  - `/workbench`
...
2. **共享壳层组件更贴近业务实际**
   - 尤其是 `EntityListTable`、`SimpleTableShell`、`QueryToolbarShell`、`CrudDialogShell`。
3. **交互细节更完整**
...
- 报表中心 → `ReportShell + ReportResultRenderer`
- 列设置逻辑 → 独立 `ColumnSettings`
- 权限按钮控制 → `PermissionAwareAction`
- 动态资源页 → `DynamicPageRenderer + metadata renderers`
...
#### 4.1.1 AppShell
- 职责：统一应用入口壳，挂载 router/store/theme/i18n/session/bootstrap/global-alerts。
- 场景：所有企业项目统一前端入口。
- 是否必须优先使用：**必须**。
- 是否允许绕过：**不允许**。
- 扩展点：branding、导航模式、feature area 注册。
- 与权限/theme/i18n/org 的关系：
  - 必须统一注入 permission、theme、locale、org-context。

#### 4.1.2 AuthLayout / MainLayout
- 职责：统一未登录壳层与登录后壳层。
- 场景：登录页、所有认证后页面。
- 是否必须优先使用：**必须**。
...
#### 4.1.4 ThemeSwitcher / LocaleSwitcher / OrgSwitcher
- 职责：统一主题、语言、组织切换。
- 场景：MainLayout 顶栏、用户设置区。
- 是否必须优先使用：**必须**。
- 是否允许绕过：**不允许自定义第二套机制**。
...
#### 4.1.6 SessionBootstrap
- 职责：启动恢复 token、locale、theme、org、用户会话、菜单权限。
- 场景：应用启动、刷新恢复、登录后初始化。
- 是否必须优先使用：**必须**。
- 是否允许绕过：**不允许**。
...
#### 4.2.5 CrudDialogShell
- 职责：统一新增/编辑/查看弹窗。
- 场景：中小复杂度新增编辑流。
- 是否必须优先使用：**必须优先**。
- 是否允许绕过：超复杂多步骤向导允许升级为 `FormShell` 独立页。
...
#### 4.2.9 DashboardCardShell
- 职责：统一工作台/仪表板卡片。
- 场景：工作台、管理驾驶舱、首页指标块。
- 是否必须优先使用：**必须优先**。
- 是否允许绕过：复杂 BI 大屏可另走可视化壳层。
...
### 5.1 登录页
- 结构：`AuthLayout + FormShell(简化版)`
- 必须使用的标准组件：AuthLayout、AppAlerts、SessionBootstrap
- 适用场景：统一登录入口
...
### 5.2 工作台页
- 结构：`MainLayout + PageSection + DashboardCardShell`
- 必须组件：PageSection、DashboardCardShell、PermissionGuard
- 场景：登录后首页、分角色首页
...
### 5.3 查询列表页
- 结构：`PageSection + QueryToolbarShell + EntityListTable`
- 必须组件：QueryToolbarShell、EntityListTable、ActionBar、Pagination
- 场景：通用资源查询
...
### 5.5 列表 + 详情页
- 结构：`MasterDetailShell` 或 `EntityListTable + DetailShell`
- 必须组件：EntityListTable、DetailShell 或 MasterDetailShell
- 场景：列表选中查看详情
...
### 5.6 主从页
- 结构：`MasterDetailShell`
- 必须组件：MasterDetailShell、SimpleTableShell / DetailShell
...
### 5.7 新增/编辑弹窗页
- 结构：`CrudDialogShell + MetadataFormRenderer/FormShell`
- 必须组件：CrudDialogShell、ActionRenderer
- 场景：中等复杂度创建/编辑
...
### 5.8 表单页
- 结构：`PageSection + FormShell`
- 必须组件：FormShell、ActionBar、PermissionGuard
...
### 5.9 只读详情页
- 结构：`PageSection + DetailShell + SimpleTableShell`
- 必须组件：DetailShell、SimpleTableShell、StatusChip
- 场景：实体详情、配置详情、报表说明页
...
### 5.10 报表页
- 结构：`PageSection + ReportShell`
- 必须组件：QueryToolbarShell、ReportShell、SimpleTableShell / MetadataTableRenderer
- 场景：统计报表、台账、对账、导出页
...
### 5.11 审批任务页
- 结构：`PageSection + QueryToolbarShell + EntityListTable + DetailShell/TaskPanel`
- 必须组件：QueryToolbarShell、EntityListTable、ActionRenderer、StatusChip
- 场景：待办、已办、审批处理
...
### 5.12 基础资料页
- 结构：`BaseDataPageShell = QueryToolbarShell + EntityListTable + CrudDialogShell`
- 必须组件：QueryToolbarShell、EntityListTable、CrudDialogShell
- 场景：客户、供应商、员工、物料等
...
### 5.13 角色用户权限页
- 结构：`MasterDetailShell` 或 `List + Detail + Assignment`
- 必须组件：EntityListTable、DetailShell、PermissionAwareAction、FormShell
...
### 5.14 组织管理页
- 结构：`MasterDetailShell + FormShell`
- 必须组件：OrgSwitcher、MasterDetailShell、FormShell
- 场景：组织树、组织详情、组织配置
...
### 5.15 系统配置页
- 结构：`PageSection + FormShell / SimpleTableShell`
- 必须组件：PageSection、FormShell、PermissionGuard
- 场景：字典配置、参数配置、功能开关
- 必须统一交互：保存前校验、差异提示、变更记录入口
- 权限要求：高权限控制
- 可扩展点：配置分组、只读审计模式
- 禁止发散点：禁止配置页使用业务自定义壳层

### 5.16 运行时元数据页面
- 结构：`DynamicPageRenderer + 标准 Shell`
- 必须组件：DynamicPageRenderer、MetadataFormRenderer、MetadataTableRenderer、ActionRenderer
- 场景：平台生成页、资源模式页、低代码页
- 必须统一交互：生成页与手写页视觉/交互一致
- 权限要求：页面、字段、动作全部 metadata + permission 驱动
- 可扩展点：resource mode override、page preset
- 禁止发散点：禁止 runtime 页面绕过统一壳层直接渲染裸结构
...
7. **运行时元数据页不是例外，必须走同一套标准壳层。**
8. **页面 PR 必须标注：页面类型、所用标准组件、是否有 override、是否有例外申请。**
9. **例外只能受控存在，且必须有到期回收计划。**
10. **任何改动标准体系的提交，必须同步更新文档、模板、lint/contract/CI。**
...
5. 运行 `lint + contract + preset check`
...
- 系统配置页 preset
- DynamicPageRenderer
- MetadataFormRenderer / MetadataTableRenderer
- ActionRenderer
...
- ReportShell + drilldown/export 规范
...
3. **先做 Shell 标准化，再做 Page Preset 标准化，再做 Runtime 标准化。**
