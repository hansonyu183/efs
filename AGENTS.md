...
- The primary runtime integration target is schema-first `user-apps/<app-name>/app.schema.ts` -> `createPlatformAppFromSchema(...)` -> `EfsApp`; `useApp()`/controller tree remains a legacy compatibility shape.
