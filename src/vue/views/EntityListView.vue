     ...
     <div v-else class="efs-resourcecrudpage__mobile-cards">
      ...
      >
       <div v-for="column in slotProps.visibleColumns" :key="column.key" class="efs-resourcecrudpage__mobile-field">
        <span class="efs-resourcecrudpage__mobile-label">{{ column.label || column.title || resolveLabel({ key: column.key, instance, namespaces: ['table.columns', 'columns', 'fields'] }) }}</span>
...
import { loadStoredAuthSession } from '../../model/app/auth-session'
...
const normalizedQueryFields = computed(() => props.queryFields.map((queryField) => ({
 key: queryField.key,
 label: resolveFieldLabel(queryField.key, queryField.label ?? queryField.title, ['resourceCrud.queryFields', 'queryFields', 'fields']),
 hint: queryField.hint ?? resolveOptionalLabel({
  key: queryField.key,
  instance,
  namespaces: ['resourceCrud.queryFieldHints', 'queryFieldHints', 'hints'],
 }),
 type: queryField.type ?? 'text',
 placeholder: queryField.placeholder ?? resolveOptionalLabel({
  key: queryField.key,
  instance,
  namespaces: ['resourceCrud.queryFieldPlaceholders', 'queryFieldPlaceholders', 'placeholders'],
 }),
 options: (queryField.options ?? []).map((option) => ({
  label: option.title ?? resolveLabel({
   key: option.key,
   instance,
...
const normalizedFormSections = computed(() => props.formSections.map((section) => ({
 key: section.key,
 title: section.title ?? resolveLabel({
  key: section.key,
  instance,
  namespaces: ['resourceCrud.formSections', 'formSections', 'sections'],
 }),
 description: section.description ?? resolveOptionalLabel({
  key: section.key,
  instance,
 ...
 fields: (section.fields ?? []).map((field) => ({
  key: field.key,
  label: resolveFieldLabel(field.key, field.label ?? field.title, ['resourceCrud.formFields', 'formFields', 'fields']),
  widget: field.widget ?? 'text',
  placeholder: field.placeholder ?? resolveOptionalLabel({
   key: field.key,
   instance,
   namespaces: ['resourceCrud.formFieldPlaceholders', 'formFieldPlaceholders', 'placeholders'],
  }),
  options: collectFormFieldOptions(field.key, field.widget ?? 'text', field.options),
 })),
})))
...
const filterScopeOptions = computed(() => ([
 ...
 ...props.columns.map((column) => ({
  key: column.key,
  label: resolveFieldLabel(column.key, column.label ?? column.title, ['table.columns', 'columns', 'fields']),
 })),
]))
...
const resolvedRowActions = computed(() => {
 ...
 return source.map((action) => ({
  ...action,
  label: action.title ?? resolveActionLabel(action.key, 'row'),
...
async function loadPermissionCatalogFromServer() {
 ...
 const data = await requestJson(
  ...
  '/agentos-api',
  {
   ...
   authScheme: session?.tokenType || 'Bearer',
  },
  {
...
function collectFormFieldOptions(fieldKey: string, widget: string, explicitOptions: Array<{ key: string, value: string, title?: string, disabled?: boolean }> = []) {
...
function normalizeResourceActions(actions: ResourceCrudAction[], scope: Extract<ResourceCrudActionScope, 'page' | 'batch'>, requireSelection: boolean) {
 return actions
  ...
  .map((action) => ({
   ...action,
   label: action.title ?? resolveActionLabel(action.key, scope),
...
function resolveFieldLabel(key: string, explicit: string | undefined, namespaces: string[]) {
 return explicit || resolveLabel({ key, instance, namespaces })
}
