 ...
 <section class="efs-reportview">
  ...
  >
   <template #header-actions>
    <ActionBar v-if="visibleActions.length > 0" :actions="visibleActions" align="end" :busy="resolvedBusy" />
    <div v-else />
   </template>
    ...
    <div class="efs-reportview__result">
     <section v-if="resolvedSummary.length > 0" class="efs-reportview__summary-grid">
      <DashboardCardPanel
       ...
       :key="metric.key"
       :title="metric.title || resolveMetricTitle(metric.key)"
       :value="metric.value ?? '-'"
...
const normalizedQueryFields = computed(() => props.queryFields.map((field) => ({
 ...
 label: field.label ?? field.title ?? resolveQueryFieldLabel(field.key),
 options: field.options?.map((option) => ({
  title: option.key ? resolveQueryOptionLabel(field.key, option.key) : option.value,
  value: option.value,
...
const normalizedColumns = computed(() => props.columns.map((column) => ({
 key: column.key,
 title: column.title ?? column.label ?? resolveColumnLabel(column.key),
...
const resolvedSummary = computed(() => {
 const controllerSummary = props.controller?.state?.summary
 if (controllerSummary) return controllerSummary
 if (viewState.summary.length > 0) return viewState.summary
...
const visibleActions = computed(() => (props.controller?.actions?.actions ?? [])
 ...
 .map((action) => ({
  key: action.key,
  label: action.title ?? resolveActionLabel(action.key),
...
async function dispatchAction(action: ReportViewAction) {
 if (action.key === 'export' || action.builtin === 'export') {
  await handleExport()
  return
 }
...
function resolveColumnLabel(key: string) {
 return resolveLabel({ key, namespaces: ['report.columns', 'columns', 'fields'] })
}
